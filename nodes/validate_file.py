...
from nodes._helpers import open_h5_temp
...
def validate_file(ax: AxiomContext, input: ValidateFileRequest) -> ValidateFileResult:
    ...
    (the HDF5 library can open it), without walking the tree or reading any
    data. Returns valid=false with a human-readable detail for anything
    unparseable — never a crash or raised error.
    """
    try:
